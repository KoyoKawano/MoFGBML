	    ...
	    if(evaluations != null) {
	    	if(evaluations % frequency == 0) {
	    		...
	    		new PittsburghSolutionListOutput(getPopulation())
	    			.printSolutionsToFile(new DefaultFileOutputContext(path, ","), population);

			    path = outputRootDir + sep + "FUN-" + evaluations + ".csv";
			    new PittsburghSolutionListOutput(getPopulation())
	        		.printObjectivesToFile(new DefaultFileOutputContext(path, ","), population);
	    	}
	    }
		else {
